...
def download_versioned_cdragon_items_bin():
    ...
    return list(filter(lambda x: ("itemID" in x), json))
...
def download_versioned_cdragon_champion_splash(champion_key, skin_id):
    ...
    return utils.download_image(constants.cdragon_url + f"/{settings.patch['cdragon']}/plugins/rcp-be-lol-game-data/global/default/v1/champion-splashes/{champion_key}/{skin_id}.jpg")
...
def download_versioned_cdragon_item_icon(item_id):
    return utils.download_image(constants.cdragon_url + f"/{settings.patch['cdragon']}/plugins/rcp-be-lol-game-data/global/default/data/items/icons2d/{item_id}.png")
